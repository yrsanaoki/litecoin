...
#if defined(NDEBUG)
# error "Poohcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd7106845c38eea44acdc3f5a70d8b37c614075ba4370e5f99214962343f1d69e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Poohcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Poohcoin Signed Message:\n";
...
{
    // Poohcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 890604 blocks, which will occur approximately every 1.74 years
    nSubsidy >>= (nHeight / 890604); // Poohcoin: 840k blocks in 1.74 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8 * 60 * 60; // Poohcoin: 8 hours
static const int64 nTargetSpacing = 1 * 60; // Poohcoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Poohcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Poohcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7b44c1e1837af532dc65819e8c9e6c5f77b27dc50a70f7f69707f70e8cfeb70a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 7/11/2021 Xi Jinping Is Rewriting China's History";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d28b94a671767c7e995ad63c6739790590a49c1a2c6b0dbdd33b92c7eefa5ef47e1aa84367e5f9b5c4542d8914435dfc6cfdde4b23642ad78c607dbdf8da6e64") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637850888;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087269504;

        if (fTestNet)
        {
            block.nTime    = 1637850872;
            block.nNonce   = 385619917;
        }
...
// Mine genesis block
    ...
    if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5e6aaa23c165954e40e85ac6c61d876eea1ff73251d7552361edc3839f5345f9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc0, 0xb0, 0xda }; // Poohcoin: P in hex 500a
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PoohcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PoohcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PoohcoinMiner(CWallet *pwallet)
{
    printf("PoohcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("poohcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PoohcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PoohcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PoohcoinMiner, pwallet));
}
